...
def test_domain_mapping_bloodhound_automatic(tmp_path):
    ...
    import sqlite3
    import os
    from adpa import init_db, parse_ntds, apply_domain_mapping, extract_bh_mapping_data

    # Use pytest tmp_path
    db_path = tmp_path / "test_bh_mapping.db"
    init_db(str(db_path), "testkey")

    # user1: Exact domain match. NTDS has LOCAL.DOMAIN. BH extracted LOCAL.DOMAIN.
    # user2: Fallback match via SID/RID. NTDS has FOO, RID 1002. BH has username user2, RID 1002, domain OTHER.DOMAIN.
    # user3: Fallback match fails (RID mismatch). NTDS has FOO, RID 1003. BH has username user3, RID 9999, domain NO.MATCH. JSON fallback applies.

    ntds_data = "LOCAL.DOMAIN\\user1:1001:aad3b435b51404eeaad3b435b51404ee:31d6cfe0d16ae931b73c59d7e0c089c0:::\nFOO\\user2:1002:aad3b435b51404eeaad3b435b51404ee:31d6cfe0d16ae931b73c59d7e0c089c0:::\nFOO\\user3:1003:aad3b435b51404eeaad3b435b51404ee:31d6cfe0d16ae931b73c59d7e0c089c0:::"
    ...
    bh_data = {
        "domains": [
            {"type": "DOMAIN", "Properties": {"name": "LOCAL.DOMAIN"}}
        ],
        "users": [
            {"type": "USER", "ObjectIdentifier": "S-1-5-21-123-1002", "Properties": {"domain": "OTHER.DOMAIN", "samaccountname": "user2"}},
            {"type": "USER", "ObjectIdentifier": "S-1-5-21-123-9999", "Properties": {"domain": "NO.MATCH", "samaccountname": "user3"}},
        ]
    }
    ...
    mapping_data = {"FOO": ["FALLBACK.DOMAIN"]}
    mapping_file = tmp_path / "test_mapping.json"
    with open(mapping_file, "w") as f:
        json.dump(mapping_data, f)

    parse_ntds(str(ntds_file), str(db_path), "testkey")
    bh_mapping_data = extract_bh_mapping_data([str(bh_file)])
    apply_domain_mapping(str(db_path), mapping_path=str(mapping_file), interactive=False, bh_mapping_data=bh_mapping_data, db_key="testkey")
    ...
    result = {(r[0].upper(), r[2].lower()): r[1].upper() for r in users}

    # user1 gets exact match on domain FQDN.
    assert result[("LOCAL.DOMAIN", "user1")] == "LOCAL.DOMAIN"

    # user2 gets match via RID fallback.
    assert result[("FOO", "user2")] == "OTHER.DOMAIN"

    # user3 fails RID fallback, falls back to JSON.
    assert result[("FOO", "user3")] == "FALLBACK.DOMAIN"
...
def test_extract_bh_mapping_data_invalid_utf8(tmp_path):
    import adpa
    bh_file = tmp_path / "bh_invalid.json"
    # Create a JSON with invalid UTF-8 byte 0x86
    with open(bh_file, "wb") as f:
        f.write(b'{"users": [{"Properties": {"domain": "TEST", "samaccountname": "user\x86"}, "ObjectIdentifier": "S-1-5-21-123-1001"}]}')

    # Should not raise exception
    valid_fqdns, bh_users_by_name = adpa.extract_bh_mapping_data([str(bh_file)])

    assert len(valid_fqdns) == 1
    assert valid_fqdns["test"] == "TEST"
    assert len(bh_users_by_name) == 1
    # Check the result has the replacement character
    user_key = list(bh_users_by_name.keys())[0]
    assert "user" in user_key
    assert bh_users_by_name[user_key][0]["rid"] == 1001
